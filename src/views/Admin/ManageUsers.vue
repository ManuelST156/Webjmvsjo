    ...
    <h1>Administrar Usuarios</h1>
        ...
        <div class="fullLine">
          <DataTable :value="userList">
            ...
            <Column :style="{ minWidth: '1rem', fontSize: '14px' }" field="esCatequista" header="Es Catequista?">
                <template #body="slotProps">
                    <div class="fullLine">
                    <Checkbox v-model="slotProps.data.esCatequista" :binary="true" :readonly="true" /> 
                    <span class="ml-2">{{ getStatusCatechist(slotProps.data.esCatequista) }}</span>
                    </div>
                </template>
            </Column>
            ...
            >
            <template #body="slotProps">
                <span class="ml-2">{{ getUserRol(slotProps.data.idRolUsuario) }}</span>
            </template>
...
const saveNewUser = async() => {
        ...
        !user.value.telefonoUsuario ) 
    {
        ...
        if (checkUser.data.length == 0) {
            ...
            const { data:userNew, error } = await supabase.auth.admin.createUser({
             email: user.value.correoUsuario,
            password: createPassword(),
            email_confirm: true  
            });

            console.log(userNew.user.id);
    ...
    else
    {
        ...
        {
            const { data, error } = await supabase.auth.admin.updateUserById(dataUsers.data[0].id, {
            email: user.value.correoUsuario,
            });
    ...
    const index = userList.value.findIndex(user => user.idUsuario === element.idUsuario); 
    if (index !== -1) {
...
const deleteUser = async () => {
    ...
    console.log(user.value.idUsuario);
    ...
    const {error:userTable}= await supabase.from("Usuarios").delete().eq('idUsuario',user.value.idUsuario);
